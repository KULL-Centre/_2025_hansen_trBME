...
class TimeResolvedBME():
    def __init__(self, esaxs_dir, meta_saxs, bfile, intname='intensity_'):
        ...
        if not os.path.isdir(self.__esaxs_dir):
            raise ValueError(
                f'Experimental intensities directory "{self.__esaxs_dir}" not found.')
        ...
        if not tmp:
            raise ValueError(
                f'No intensity file found in "{self.__esaxs_dir}".')
        else:
            self.__nframes = len(tmp)
            for i in range(self.__nframes):
                self.__esaxs.append(
                    f'{self.__esaxs_dir}/{intname}{i}.dat')  # needed for BME
                # needed for FP and plotting
                self.__exp_ints.append(self.quick_load(self.__esaxs[i]))

        if not os.path.isfile(self.__meta_saxs):
            raise ValueError(f'BME file "{self.__meta_saxs}" not found.')
        else:
            # needed for BME and plotting
            self.__data = self.quick_load(self.__meta_saxs)
        ...
        if not os.path.isfile(bfile):
            ...
            self.w0 = w[:]
            # static prior weights for reweighting (only for proof of concept!!)
    ...
    def setup_fes(self, T, kde=False, scale=1, s=0, bins=50, mesh_points=501, box_size=300):
        ...
        if kde:
            ...
            h, _ = np.histogram(
                self.__cv, weights=self.__wmetad, bins=int(bins))
        ...
        xnm = np.linspace(-box_size / 2. * self.__NM,
                          box_size / 2 * self.__NM, int(mesh_points))
        func = sp.interpolate.splev(
            cv_mesh, tck_fes, der=0) / np.max(fes) * AVOGADRO  # free energy in J
    ...
    def fpsolve(self, p0_loc, temp, drag=1e-6, t=10, npriors=None):
        ...
        if not fplib:
            raise ImportError(
                "The fplanck library is missing. Please, install it via: 'pip install fplanck'.")
        if not self.__fes_is_setup:
            raise FESNotSetUP(
                'Free energy surface has not been set up yet. Run setup_fes() first.')
    ...
    def optimise_FP_params(self, times, temps, p0_loc, drag=1e-6, npriors=None, start=0, stride=1, end=-1):
        ...
        for j, temp in enumerate(temps):
            for n, time in enumerate(times):
                print("# SOLVING FP: TEMP={i}K, TIME={n}\r".format(
                    i=temp, n=time), end="")
                # solve FP and convert pdfs to weights
                self.fpsolve(p0_loc, drag=drag, t=time,
                             npriors=npriors, temp=temp)
                ...
                for i in range(self.__nframes):
                    expi = expis[i]
                    prior = np.sum(
                        data * self.priors[:, i][:, np.newaxis], axis=0)
                    x2 += np.average(((prior[1:] -
                                     expi[:, 1])/expi[:, 2]) ** 2)
    ...
    def FP2weights(self, start=0, stride=1, end=-1, n_mesh=300):
        ...
        mesh = np.linspace(self.__cv.min(), self.__cv.max(), n_mesh)
        for i in np.arange(start, end, stride):
            interp_prob = sp.interpolate.splrep(mesh, self.__pt[i], s=0)
            # associates weights to q values
            prior = np.abs(sp.interpolate.splev(self.__cv, interp_prob, der=0))
            prior = prior / np.sum(prior)  # normalize the weights to unity
            prior = prior.reshape(len(prior), 1)
            priors.append(prior)
        # concatenate all priors into an array
        self.priors = np.concatenate(priors, axis=1)
    ...
    def optimise_priors_to_exp(self, s=0.1, mode='max'):
        ...
        if self.__computed_priors or self.__loaded_priors:
            ...
            opts, chis = [], []
            print("# PRE-COMPUTING PRIOR AVERAGES ...")
            priors = [np.average(self.__data, weights=i, axis=0)
                      for i in self.priors.T]
            for n, j in enumerate(self.__exp_ints):
                x2s = []
                for p in priors[start:]:
                    x2 = np.average(((j[:, 1] - p[1:]) / j[:, 2]) ** 2)
                ...
                if mode == 'min':
                    ...
                    relevant_x2 = [idx for idx in np.argsort(
                        x2s) if x2s[idx] <= threshold]
                    if mode == 'max':
                        opt = np.max(relevant_x2) + start
                    elif mode == 'average':
                        index = np.argmin(
                            np.abs(np.array(relevant_x2) - np.average(relevant_x2)))
                        opt = index + start
                    else:
    ...
    def crossval_thetas(self, w0, theta_range, nfold=2):
        ...
        for n, f in enumerate(self.__esaxs):
            if len(np.shape(w0)) == 2:
                ...
                raise ValueError(
                    f'Non compatible shape of initial weights, {np.shape(w0)}.')
    ...
    def elbow_thetas(self, default_idx=None):
        ...
        for i in range(self.__nframes):
            try:
                kneedle = KneeLocator(
                    self.res['phi'][i], self.res['x2f'][i], curve="convex", direction="increasing")
    ...
    def smooth_thetas(self, window_size=11, polyorder=1):
        ...
        self.smooth_ths = signal.savgol_filter(
            self.opt_ths, window_size, polyorder)
    ...
    def reweight_optimised(self, w0, thetas):
        ...
        self.res = {'x2i': [], 'x2f': [], 'phi': [], 'w': []}
        for n, f in enumerate(self.__esaxs):
            print("# REWEIGHTING {i}/{n}\r".format(i=n +
                  1, n=self.__nframes), end="")
            if len(np.shape(w0)) == 2:
                w = w0[:, n]
            elif len(np.shape(w0)) == 1:
                w = w0[:]
                ...
                raise ValueError(
                    'Weights array cannot be more than 2 dimensional')
    ...
    def pdens_per_frame(self, obs, idx_ths, prior_weights, bins=50):
        ...
        if len(np.shape(prior_weights)) == 1:
            hpr, _ = np.histogram(obs, bins=bins, density=True,
                                  weights=prior_weights, range=(obs.min(), obs.max()))
    ...
    def get_fes(self, mesh_points=501):
        ...
        """
        if self.__fes_is_setup:
            x = np.linspace(-self.__box_size / 2. * self.__NM,
                            self.__box_size / 2 * self.__NM, int(mesh_points))
    ...
    def plot_dynamical_priors(self, ini=0, fin=0, distance=False, outfig=None):
        ...
        if list(self.__pt):
            ...
            for i in np.arange(int(ini), len(self.__pt) - int(fin), 1):
                ax.plot(x, self.__pt[i], color=cmap.to_rgba(i))
            ax.set_xlabel('x [nm]')
            ...
            if distance:
                ax1.plot(self.diff_from_eq)
                ax1.set_xlabel('Steps')
                ax1.set_ylabel(r'$||p(t)-p_0||$')
            plt.tight_layout()
    ...
    def plot_priors_with_fes(self, cv_label='', dt=1, time_unit='ns', outfig=None):
        ...
        if list(self.__pt):
            ...
            cbar = plt.colorbar(cmap, orientation='horizontal', cax=cax)
    ...
    def plot_saxs_fits(self, frames, thidx, w0=[], dt=0.5, time_unit='ns', xlog=True, legend=False, outfig=None):
        ...
        if len(np.shape(w0)) == 1 and w0 != []:
            prior = np.average(self.__data, weights=w0,
                               axis=0)  # constant prior
        if len(frames) > 3:
            # keep only the firtst 3 frames for plotting reasons
            frames = frames[:3]
        ...
        for n, i in enumerate(frames):
            ...
            posterior = np.average(
                self.__data, weights=self.res['w'][i][thidx[n]], axis=0)
            if len(np.shape(w0)) == 2:
            ...
            if w0 != []:
                ax1.plot(exp[:, 0] * 10, prior[1:], lw=2,
                         label='Prior', c='tab:blue', zorder=1)
            ax1.plot(exp[:, 0] * 10, posterior[1:], lw=2,
